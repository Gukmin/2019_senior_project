...
class DQNAgent:
    def __init__(self, action_size, state_size=(84, 84, 4)):
        ...
        #그레디언트 클리핑, 그레이던트 너무 크면 변화심해서 후버로스도
        # 타깃 모델 초기화
        self.update_target_model()

        self.avg_q_max, self.avg_loss = 0, 0

        self.writer = tf.summary.create_file_writer('summary/mario_dqn')
...
if __name__ == "__main__":
    # 환경과 DQN 에이전트 생성
    actionSet = actions.SIMPLE_MOVEMENT
    env = gym_super_mario_bros.make('SuperMarioBros-v0')
    env = JoypadSpace(env,actionSet)
    #env.action_space
    agent = DQNAgent(action_size=len(actionSet))
    ...
    for e in range(num_episode):
        ...
        while not done:
            ...
            observe, reward, done, info = env.step(action) #환경이 observe, reward, 끝났나, 목숨개수 줌
            # 각 타임스텝마다 상태 전처리 이전꺼 버리고 최신4개
